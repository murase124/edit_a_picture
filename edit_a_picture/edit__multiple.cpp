...
//内
void edit_main_multiple(imgdate** pimg);	//オーバーライド 一括
void Polygon_multiple(imgdate** pimg);		//多角形
void line_multiple(imgdate** pimg);			//線
void mosaic_multiple(imgdate** pimg);		//モザイク
void Rectangle_multiple(imgdate** pimg);	//四角形
void Text_multiple(imgdate** pimg);			//文字
...
//多角形
void Polygon_multiple(imgdate** pimg)
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		scanf_s("%d", &beside[0][i]);
		if (beside[0][i] == -1) break;
		printf("edit_Viewでの高さの座標\n");
		scanf_s("%d", &vertical[0][i]);
		gomi();
	}
	...
	if (i > 2 && (flg == 0 || flg == 1))
	{
		...
		if (flg == 0)
		{
			for (int loop = 0; loop < *((*pimg)->number); loop++)
			{
				for (int j = 0; j < i; j++)
				{
					vertical[1][j] = vertical[0][j] * (*(*pimg)->Change_img)->img->rows / 500.0;
					beside[1][j] = beside[0][j] * (*(*pimg)->Change_img)->img->rows / 500.0;
					pt[j] = cv::Point(beside[1][j], vertical[1][j]);
				}
...
//線
void line_multiple(imgdate * *pimg)
{
	...
	for (int i = 0; i < *((*pimg)->number); i++)
	{
		...
		//cv::fillConvexPoly(*((*(*pimg)->Change_img)->img), pt, 2, colour);//BGR
...
//モザイク
void mosaic_multiple(imgdate** pimg)
{
	...
	for (int loop = 0; loop < *((*pimg)->number); loop++)
	{
		...
		//描画
		for (j = x_y2[0][1]; j < x_y2[1][1]; j = j + range_box)
		{
			//範囲オーバー
			if (j + range_box > x_y2[1][1])	j = x_y2[1][1] - range_box;
			for (i = x_y2[0][0]; i < x_y2[1][0]; i = i + range_box)
			{
				//範囲オーバー
				if (i + range_box > x_y2[1][0])  i = x_y2[1][0] - range_box;
...
//四角形
void Rectangle_multiple(imgdate** pimg)
{
	srand((unsigned int)time(NULL)); // 現在時刻の情報で初期化
	cv::Scalar colour;
	int x_y[2][2];
...
//文字
void Text_multiple(imgdate** pimg)
{
	srand((unsigned int)time(NULL)); // 現在時刻の情報で初期化
	cv::Scalar colour;
	int x_y[2];
